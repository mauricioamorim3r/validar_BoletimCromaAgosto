...
from config import DEBUG, HOST, PORT, LIMITES_AGA8
...
def calcular_propriedades_fluido(componentes_data):
    ...
    gas_constant = 8314.47  # J/(kmol·K)
    temperature_k = 293.15  # K (20°C)

    massa_especifica = (
        pressure_pa * massa_molecular /
        (propriedades['Fator de Compressibilidade'] * gas_constant * temperature_k)
    )
...
def _fetch_relatorio_data(boletim_id):
    db = get_db()
...
def _add_header_section(story, styles):
    story.append(Paragraph('BRAVA ENERGIA', styles['titulo']))
    story.append(
        Paragraph(
...
def _format_prazo_dias(entry):
    valor = entry.get('dias_decorridos') if entry else None
    return f'{valor} dias' if valor is not None else 'N/A'


def _format_prazo_status(entry):
...
def _build_prazos_table(boletim):
    try:
        ...
        prazos_data = [
            ...
            ['Coleta -> Emissao', _format_prazo_dias(coleta_emissao),
             '25 dias (Portaria 52 ANP)', _format_prazo_status(coleta_emissao)],
            ['Emissao -> Validacao', _format_prazo_dias(emissao_validacao),
             '1 dia (Portaria 52 ANP)', _format_prazo_status(emissao_validacao)],
            ['Prazo Total (Coleta -> Validacao)', _format_prazo_dias(prazo_total),
             '28 dias (Portaria 52 ANP)', _format_prazo_status(prazo_total)],
        ]
    except Exception:
        prazos_data = [
            ['Validação', 'Status'],
    ...
    col_widths = [120, 80, 120, 80] if len(prazos_data[0]) == 4 else [200, 150]
    prazos_table = Table(prazos_data, colWidths=col_widths)
...
def _build_condicoes_table(boletim):
    condicoes_data = [
    ...
    return condicoes_table


def _build_componentes_table(componentes):
    comp_data = [
        ['Componente', 'Unidade', '% Molar', 'Status A.G.A #8', 'Status CEP', 'Min A.G.A', 'Max A.G.A'],
    ]
    for comp in componentes:
        nome_simples = comp['nome'].split(',')[0]
        limites = LIMITES_AGA8.get(nome_simples, {'min': 0, 'max': 100})
        comp_data.append([
            comp['nome'],
            '% mol',
...
def _build_propriedades_table(propriedades):
    ...
    for prop in propriedades:
        ...
        for key, val in limites_propriedades.items():
            if key in prop['nome']:
                ...
                break
        if limite_info:
            unidade = limite_info['unidade']
            lim_inf = str(limite_info['min'])
            lim_sup = str(limite_info['max'])
        else:
            unidade = '-'
            lim_inf = '-'
            lim_sup = '-'
...
def _build_checklist_table(checklist):
    checklist_data = [['Item', 'Descrição', 'Situação', 'N/A', 'Observações']]
    for item_row in checklist:
        item = dict(item_row)
        descricao = item['descricao']
        descricao = descricao[:60] + '...' if len(descricao) > 60 else descricao
...
def _build_resultado_table(boletim):
    resultado_data = [
        ['Validação', 'Status'],
...
def _build_responsaveis_table(boletim):
    resp_data = [
...
def _add_informacoes_section(story, boletim, styles):
    story.append(Paragraph('1. INFORMAÇÕES DO BOLETIM', styles['secao']))
    story.append(_build_info_table(boletim))


def _add_prazos_section(story, boletim, styles):
    story.append(Spacer(1, 15))
    story.append(Paragraph('VALIDAÇÃO DE PRAZOS ANP - REGULAMENTO TÉCNICO 52', styles['secao']))
    story.append(_build_prazos_table(boletim))


def _add_condicoes_section(story, boletim, styles):
    story.append(Spacer(1, 15))
    story.append(Paragraph('2. CONDIÇÕES DO PROCESSO', styles['secao']))
    story.append(_build_condicoes_table(boletim))


def _add_componentes_section(story, componentes, styles):
    story.append(Spacer(1, 15))
    story.append(Paragraph('3. COMPONENTES GASOSOS (%)', styles['secao']))
    if componentes:
        story.append(_build_componentes_table(componentes))
    else:
        story.append(Paragraph('Nenhum componente cadastrado para este boletim.', styles['normal']))


def _add_propriedades_section(story, propriedades, styles):
    story.append(Spacer(1, 15))
    story.append(Paragraph('4. PROPRIEDADES DO FLUIDO', styles['secao']))
    if propriedades:
        story.append(_build_propriedades_table(propriedades))
    else:
        story.append(Paragraph('Nenhuma propriedade cadastrada para este boletim.', styles['normal']))


def _add_checklist_section(story, checklist, styles):
    story.append(Spacer(1, 15))
    story.append(Paragraph('5. CHECK LIST NBR ISO/IEC 17025', styles['secao']))
    if checklist:
        story.append(_build_checklist_table(checklist))
    else:
        story.append(Paragraph('Checklist não preenchido para este boletim.', styles['normal']))


def _add_resultado_section(story, boletim, styles):
    story.append(Spacer(1, 20))
    story.append(Paragraph('6. RESULTADO FINAL DA VALIDAÇÃO', styles['secao']))
    story.append(_build_resultado_table(boletim))


def _add_observacoes_section(story, boletim, styles):
    if boletim['observacoes']:
        story.append(Spacer(1, 15))
        story.append(Paragraph('7. OBSERVAÇÕES', styles['secao']))
        story.append(Paragraph(str(boletim['observacoes']), styles['normal']))


def _add_responsaveis_section(story, boletim, styles):
    story.append(Spacer(1, 20))
    story.append(Paragraph('8. RESPONSÁVEIS PELA VALIDAÇÃO', styles['secao']))
    story.append(_build_responsaveis_table(boletim))
...
def gerar_pdf_relatorio(boletim_id):
    dados = _fetch_relatorio_data(boletim_id)
    if not dados:
        return None

    doc, buffer = _build_pdf_document()
    styles = _build_pdf_styles()
    story = []

    _add_header_section(story, styles)
    boletim = dados['boletim']
    _add_informacoes_section(story, boletim, styles)
    _add_prazos_section(story, boletim, styles)
    _add_condicoes_section(story, boletim, styles)
    _add_componentes_section(story, dados['componentes'], styles)
    _add_propriedades_section(story, dados['propriedades'], styles)
    _add_checklist_section(story, dados['checklist'], styles)
    _add_resultado_section(story, boletim, styles)
    _add_observacoes_section(story, boletim, styles)
    _add_responsaveis_section(story, boletim, styles)
...
def _parse_boletim_form(form):
    dados = {
        'numero_boletim': form['numero_boletim'],
        'data_coleta': form['data_coleta'],
        'data_recebimento': form['data_recebimento'],
        'data_analise': form['data_analise'],
        'data_emissao': form['data_emissao'],
        'identificacao_instalacao': form['identificacao_instalacao'],
        'agente_regulado': form['agente_regulado'],
        'responsavel_amostragem': form['responsavel_amostragem'],
        'pressao': float(form['pressao']),
        'temperatura': float(form['temperatura']),
        'observacoes': form['observacoes'],
        'responsavel_tecnico': form['responsavel_tecnico'],
        'responsavel_elaboracao': form['responsavel_elaboracao'],
        'responsavel_aprovacao': form['responsavel_aprovacao'],
    }
    erros = _validar_datas_boletim(dados, form)
    if erros:
        return dados, erros
    dados.update({
        'numero_documento': f"{dados['numero_boletim']}/2025",
        'plataforma': form.get('plataforma', 'FPSO ATLANTE'),
        'sistema_medicao': form.get('sistema_medicao', 'GÁS COMBUSTÍVEL LP'),
        'classificacao': form.get('classificacao', 'FISCAL'),
        'ponto_coleta': form.get('ponto_coleta', 'LP FUEL GAS'),
    })
    return dados, []


def _validar_datas_boletim(dados, form):
    erros = []
    data_coleta = datetime.strptime(dados['data_coleta'], '%Y-%m-%d').date()
    data_emissao = datetime.strptime(dados['data_emissao'], '%Y-%m-%d').date()

    if data_emissao < data_coleta:
        erros.append(
            f"Data de emissão ({data_emissao.strftime('%d/%m/%Y')}) não pode ser anterior à data de coleta ({data_coleta.strftime('%d/%m/%Y')})"
        )

    dias_coleta_emissao = (data_emissao - data_coleta).days
    if dias_coleta_emissao > 25:
        erros.append(
            f"Prazo entre coleta e emissão excede 25 dias (RTM 52): {dias_coleta_emissao} dias"
        )

    data_validacao_raw = form.get('data_validacao')
    if data_validacao_raw and data_validacao_raw.strip():
        data_validacao = datetime.strptime(data_validacao_raw, '%Y-%m-%d').date()

        if data_validacao < data_emissao:
            erros.append(
                f"Data de validação ({data_validacao.strftime('%d/%m/%Y')}) não pode ser anterior à data de emissão ({data_emissao.strftime('%d/%m/%Y')})"
            )

        if data_validacao < data_coleta:
            erros.append(
                f"Data de validação ({data_validacao.strftime('%d/%m/%Y')}) não pode ser anterior à data de coleta ({data_coleta.strftime('%d/%m/%Y')})"
            )

        dias_total = (data_validacao - data_coleta).days
        if dias_total > 28:
            erros.append(
                f"Prazo total entre coleta e validação excede 28 dias (RTM 52): {dias_total} dias"
            )

    return erros


def _insert_boletim(db, dados):
    db.execute(
        '''
        ...
        ''', (
            dados['numero_boletim'], dados['numero_documento'],
            dados['data_coleta'], dados['data_recebimento'],
            dados['data_analise'], dados['data_emissao'],
            dados['identificacao_instalacao'], dados['plataforma'],
            dados['sistema_medicao'], dados['classificacao'],
            dados['ponto_coleta'], dados['agente_regulado'],
            dados['responsavel_amostragem'], dados['pressao'], dados['temperatura'],
            dados['observacoes'], dados['responsavel_tecnico'], dados['responsavel_elaboracao'],
            dados['responsavel_aprovacao'], None, None, None, None, None,
        ),
    )
...
def _processar_componentes(db, boletim_id, data_coleta, form):
    for componente in COMPONENTES_PADRAO:
        valor = float(form[componente])
        status_aga = 'VALIDADO' if valida_aga8(componente, valor) else 'INVALIDADO'
        historico = get_historico_componente(componente)
        status_cep = 'VALIDADO' if valida_cep(componente, valor, historico) else 'INVALIDADO'
...
def _processar_propriedades(db, boletim_id, data_coleta, form):
    campos_propriedades = {
        'fator_compressibilidade': 'Fator de compressibilidade',
        'massa_especifica': 'Massa Específica',
        'massa_molecular': 'Massa Molecular',
    }

    inseriu = False
    for campo, nome in campos_propriedades.items():
        valor_bruto = form.get(campo)
        if not valor_bruto or not valor_bruto.strip():
            continue

        valor = float(valor_bruto)
        status_cep = 'VALIDADO' if valida_cep_propriedade(nome, valor) else 'INVALIDADO'

        db.execute(
            'INSERT INTO propriedades (boletin_id, nome, valor, status_aga, status_cep) '
            'VALUES (?, ?, ?, ?, ?)',
            (boletim_id, nome, valor, 'N/A', status_cep),
        )

        db.execute(
            'INSERT INTO historico_propriedades (propriedade, boletin_id, valor, data_coleta) '
            'VALUES (?, ?, ?, ?)',
            (nome, boletim_id, valor, data_coleta),
        )
...
def _atualizar_status_boletim(db, boletim_id):
    componentes_aga_invalidos = db.execute(
        'SELECT COUNT(*) FROM componentes WHERE boletin_id = ? AND status_aga = ?',
        (boletim_id, 'INVALIDADO'),
    ).fetchone()[0]
    status_aga8 = 'INVALIDADO' if componentes_aga_invalidos > 0 else 'VALIDADO'

    componentes_cep_invalidos = db.execute(
        'SELECT COUNT(*) FROM componentes WHERE boletin_id = ? AND status_cep = ?',
        (boletim_id, 'INVALIDADO'),
    ).fetchone()[0]
    status_cep = 'INVALIDADO' if componentes_cep_invalidos > 0 else 'VALIDADO'

    status_checklist = 'VALIDADO'
    status_geral = 'INVALIDADO' if (status_aga8 == 'INVALIDADO' or status_cep == 'INVALIDADO') else 'VALIDADO'
    data_validacao = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    db.execute(
        'UPDATE boletins SET status = ?, status_cep = ?, status_aga8 = ?, status_checklist = ?, data_validacao = ? '
        'WHERE id = ?',
        (status_geral, status_cep, status_aga8, status_checklist, data_validacao, boletim_id),
    )
    db.commit()
...
def cadastrar():
    ...
    if request.method == 'POST':
        dados_boletim, erros = _parse_boletim_form(request.form)
        if erros:
        ...
        db = get_db()
        boletim_id = _insert_boletim(db, dados_boletim)
        _processar_componentes(db, boletim_id, dados_boletim['data_coleta'], request.form)
        _processar_propriedades(db, boletim_id, dados_boletim['data_coleta'], request.form)
        criar_checklist_padrao(boletim_id)
        _atualizar_status_boletim(db, boletim_id)
...
COMPONENT_DB_TO_FORM = {db_name: form_name for form_name, db_name in COMPONENT_FORM_TO_DB.items()}
...
def _update_boletim_from_form(db, boletim_id, form, pressao_val, temperatura_val):
    logger.debug(
        f"Updating boletim {boletim_id} - Classificacao: {form.get('classificacao', 'N/A')}"
    )
    ...
    ''', (
        form['numero_boletim'],
        form['numero_documento'] or None,
        form['data_coleta'],
        form['data_recebimento'] or None,
        form['data_analise'] or None,
        form['data_emissao'],
        form['data_validacao'] or None,
        form['identificacao_instalacao'],
        form['plataforma'],
        form['sistema_medicao'],
        form['classificacao'],
        form['ponto_coleta'],
        form['agente_regulado'],
        form['responsavel_amostragem'],
        pressao_val,
        temperatura_val,
        boletim_id,
    ))
    ...
    verificacao = db.execute(
        'SELECT classificacao FROM boletins WHERE id = ?',
        (boletim_id,),
    ).fetchone()
    logger.debug(
        "Data verification before commit - Classificacao: %s",
        verificacao['classificacao'] if verificacao else 'None',
    )


def _update_checklist_items(db, boletim_id, form):
    checklist_items = db.execute(
        'SELECT item_numero FROM checklist_itens WHERE boletin_id = ?',
        (boletim_id,),
    ).fetchall()
    ...
    for item in checklist_items:
        ...
        if situacao_key in form:
            ...
            ''', (
                form[situacao_key],
                int(form.get(nao_aplicavel_key, 0)),
                form.get(observacao_key, '') or None,
                boletim_id,
                item_num,
            ))


def _upsert_componentes_from_form(db, boletim_id, form):
    for form_name, db_name in COMPONENT_FORM_TO_DB.items():
        percentual_raw = form.get(form_name, '').strip()
        if not percentual_raw:
            continue
        try:
        ...
        existing = db.execute(
            'SELECT id FROM componentes WHERE boletin_id = ? AND nome = ?',
            (boletim_id, db_name),
        ).fetchone()

        if existing:
            db.execute(
                'UPDATE componentes SET percentual_molar = ? WHERE id = ?',
...
def _rollback_and_redirect(db, boletim_id, message):
    db.rollback()
    db.close()
    flash(message)
...
def _process_relatorio_post(boletim_id):
    logger.debug(
        f"POST request received for boletim {boletim_id}"
    )
    logger.debug(f"Form keys: {list(request.form.keys())[:5]}...")
    logger.debug(f"Form data length: {len(request.form)}")

    db = get_db()
    try:
        pressao_val = _parse_optional_float(request.form.get('pressao'))
        temperatura_val = _parse_optional_float(request.form.get('temperatura'))

        _update_boletim_from_form(db, boletim_id, request.form, pressao_val, temperatura_val)
        _update_checklist_items(db, boletim_id, request.form)
        _upsert_componentes_from_form(db, boletim_id, request.form)
        ...
        logger.debug(
            "Data verification after commit - Classificacao: %s",
            verificacao_final['classificacao'] if verificacao_final else 'None',
        )
        ...
        return _rollback_and_redirect(db, boletim_id, f'Erro nos dados fornecidos: {str(exc)}')

    except sqlite3.Error as exc:
        return _rollback_and_redirect(db, boletim_id, f'Erro no banco de dados: {str(exc)}')

    except Exception as exc:
        return _rollback_and_redirect(db, boletim_id, f'Erro ao atualizar boletim: {str(exc)}')


def _build_component_template_dict(componentes):
    componentes_dict = {}
    for comp in componentes:
        nome_template = COMPONENT_DB_TO_FORM.get(comp['nome'])
        if nome_template:
            componentes_dict[nome_template] = comp['percentual_molar']
    return componentes_dict


def _calculate_component_limits(componentes):
    componentes_with_limits = []
    for comp in componentes:
        historico = get_historico_componente(comp['nome'])
        lci = lcs = None
        if len(historico) >= 2:
            from config import CEP_AMOSTRAS_MIN, CEP_D2_CONSTANT, CEP_SIGMA_LIMIT
            ...
            if len(ultimas_amostras) >= 2:
                media = sum(ultimas_amostras) / len(ultimas_amostras)
                amplitudes = [abs(ultimas_amostras[i] - ultimas_amostras[i - 1]) for i in range(1, len(ultimas_amostras))]
                media_amplitudes = sum(amplitudes) / len(amplitudes)
                lcs = media + CEP_SIGMA_LIMIT * media_amplitudes / CEP_D2_CONSTANT
                lci = media - CEP_SIGMA_LIMIT * media_amplitudes / CEP_D2_CONSTANT
        comp_dict = dict(comp)
    ...
    return componentes_with_limits


def _load_relatorio_context(boletim_id, edit_mode):
    db = get_db()
    try:
        boletim = db.execute(
        ...
        return {
            'boletim': boletim,
            'componentes': _calculate_component_limits(componentes),
            'propriedades': propriedades,
            'checklist': checklist,
            'componentes_dict': _build_component_template_dict(componentes),
            'edit_mode': edit_mode == 'edit',
...
def relatorio(boletim_id, edit_mode=None):
    ...
    logger.debug(
        f"Relatório acessado - Method: {request.method}, Edit Mode: {edit_mode}, ID: {boletim_id}"
    )

    if request.method == 'POST' and edit_mode == 'edit':
        return _process_relatorio_post(boletim_id)

    context = _load_relatorio_context(boletim_id, edit_mode)
    if context is None:
        flash('Boletim não encontrado!')
        return redirect(url_for('listar_boletins'))

    return render_template('relatorio_excel.html', **context)
