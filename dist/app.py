...
def gerar_pdf_relatorio(boletim_id):
    ...
    checklist = db.execute('SELECT * FROM checklist_itens WHERE boletin_id = ? ORDER BY item_numero', (boletim_id,)).fetchall()
    ...
    info_data = [
        ...
        ['Responsável:', boletim['responsavel_amostragem'], 'Pressão:', f"{boletim['pressao']} atm" if boletim['pressao'] else ''],
    ...
    for comp in componentes:
        comp_data.append([
            ...
            '100%' if comp['nome'] not in ['Propano', 'i-Butano', 'n-Butano', 'i-Pentano', 'n-Pentano', 'Oxigênio'] else ('12%' if comp['nome'] == 'Propano' else ('6%' if 'Butano' in comp['nome'] else ('4%' if 'Pentano' in comp['nome'] else '21%')))
        ])
    ...
    resultado_data = [
        ['CEP', 'A.G.A #8', 'CHECKLIST'],
        [boletim['status_cep'] or 'PENDENTE', boletim['status_aga8'] or 'PENDENTE', boletim['status_checklist'] or 'PENDENTE']
    ]
...
def dashboard():
    ...
    if where_clause:
        ...
        stats['boletins_validados'] = db.execute('SELECT COUNT(*) FROM boletins WHERE status = ?', ('VALIDADO',)).fetchone()[0]
        stats['boletins_invalidados'] = db.execute('SELECT COUNT(*) FROM boletins WHERE status = ?', ('INVALIDADO',)).fetchone()[0]
        stats['aga8_aprovados'] = db.execute('SELECT COUNT(*) FROM boletins WHERE status_aga8 = ?', ('VALIDADO',)).fetchone()[0]
        stats['cep_aprovados'] = db.execute('SELECT COUNT(*) FROM boletins WHERE status_cep = ?', ('VALIDADO',)).fetchone()[0]
        stats['checklist_aprovados'] = db.execute('SELECT COUNT(*) FROM boletins WHERE status_checklist = ?', ('VALIDADO',)).fetchone()[0]
    
    stats['taxa_validacao'] = (stats['boletins_validados'] / stats['total_boletins'] * 100) if stats['total_boletins'] > 0 else 0
    ...
    hist_query = f'''
        ...
        LIMIT 50
    '''
    historico_grafico = db.execute(hist_query, hist_params).fetchall()
...
def revalidar_boletim(boletim_id):
    ...
    for componente in componentes:
        ...
        status_cep = "VALIDADO" if valida_cep(componente['nome'], componente['percentual_molar'], historico) else "INVALIDADO"
...
def revalidar_todos_boletins():
    ...
    flash(f'Processados {total_processados} boletins: {contador_validados} validados, {contador_invalidados} invalidados')
    return redirect(url_for('listar_boletins'))
...
def relatorio_pdf(boletim_id):
    ...
    try:
        ...
        db = get_db()
        boletim = db.execute('SELECT numero_boletim FROM boletins WHERE id = ?', (boletim_id,)).fetchone()
        filename = f"Boletim_{boletim['numero_boletim'].replace('/', '_')}.pdf" if boletim else f"Boletim_{boletim_id}.pdf"
    ...
    if arquivo and allowed_file(arquivo.filename):
        try:
            ...
            if resultado['success'] > 0:
                flash(f'✅ Importação concluída: {resultado["success"]} boletins processados com sucesso!')
                
                for boletim in resultado['boletins_processados']:
                    flash(f'📋 Boletim {boletim["numero_boletim"]}: {boletim["status"]} ({boletim["componentes_processados"]} componentes)')
...
def processar_boletins_existentes():
    ...
    try:
        ...
        db.commit()
        flash(f'✅ Processados {contador} boletins existentes. Agora eles aparecem na aba Histórico e podem ser revalidados.')
